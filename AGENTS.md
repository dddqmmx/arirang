...
- Xposed scope is defined in `app/src/main/res/values-en/arrays.xml` (`xposedscope`). Required scope includes System/Android framework, `com.android.phone`, and `com.google.android.gms` per README.
...
- `my-release-key.jks` exists at repo root but is **not referenced by the build file**; release signing is manual/unconfigured.
