...
if [ "$ENABLED" = "true" ]; then
    if [ "$DEVICE_INFO_ENABLED" = "true" ]; then
        ...
        # WITHOUT exposing any platform-revealing property.
        # We cannot bind-mount the entire directory because it breaks SELinux and security checks for other HALs like gralloc-mapper.
        # We cannot create new files (like audio.primary.husky.so) using file bind-mounts.
        # So we bind-mount the real platform HAL over the 'default.so' stub.
        # When audioserver fails to find audio.primary.husky.so, it will fall back to audio.primary.default.so
        # and successfully load the real HAL!
        if [ -n "$BOARD" ]; then
            ...
            if [ -n "$REAL_PLATFORM" ] && [ "$REAL_PLATFORM" != "$BOARD" ]; then
                for hwdir in /vendor/lib64/hw /vendor/lib/hw; do
                    if [ ! -d "$hwdir" ]; then continue; fi
                    
                    for real_so in "$hwdir"/audio.*."$REAL_PLATFORM".so; do
                        [ -e "$real_so" ] || continue
                        module_type=$(basename "$real_so" | cut -d. -f2)
                        default_so="$hwdir/audio.${module_type}.default.so"
                        
                        if [ -e "$default_so" ]; then
                            if mount --bind "$real_so" "$default_so"; then
                                log -p i -t arirang_post_fs_data "audio HAL: bind mounted $real_so over $default_so"
                            else
                                log -p e -t arirang_post_fs_data "audio HAL: failed to bind mount $real_so over $default_so"
                            fi
                        fi
                    done
                done
