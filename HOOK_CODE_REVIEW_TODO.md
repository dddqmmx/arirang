...
- [ ] Fix App Set callback `Parcel` corruption and leak.
...
- [ ] Add cycle protection to recursive telephony object rewriting.
...
- [ ] Stop returning `List` for Bluetooth APIs that may declare `Set`.
...
- [ ] Add client-side timeout or async handling for clipboard permission requests.
...
- [ ] Fix SettingsProvider Bluetooth fallback prefs name.
...
- [ ] Make Wi-Fi scan-result wrapping fail closed to the original result, not `null`.
...
- [ ] Narrow GMS Advertising ID string getter hooks.
...
- [ ] Add visited-set protection to location caller resolution.
...
- [ ] Resolve per-package location profiles for shared UIDs more deterministically.
...
- [ ] Remove sensitive identifiers from logs or gate them behind debug-only logging.
...
- [ ] Avoid forcing config reload on every telephony system-property read.
...
- [ ] Do not clear the global Arirang service for every config read failure.
...
- [ ] Review SIM slot normalization semantics.
...
- [ ] Validate configured Wi-Fi/Bluetooth addresses before using them in framework objects.
...
- [ ] Remove or wire the unreachable app package branch in location hook.
...
- [ ] Revisit template inheritance mode handling in package-list visibility.
...
- [ ] Replace hard-coded Bluetooth MAC constants with config or documented policy.
...
- [ ] Clean up debug scaffolding constants.
