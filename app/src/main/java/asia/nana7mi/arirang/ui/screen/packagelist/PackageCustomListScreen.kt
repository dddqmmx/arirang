...
import asia.nana7mi.arirang.ui.component.packagelist.TemplateModeSelector
...
internal fun PackageCustomListScreen(
    ...
    // Null for the per-app target, which has no list mode of its own.
    var listMode by remember { mutableStateOf<TemplateListMode?>(null) }
    var savedListMode by remember { mutableStateOf<TemplateListMode?>(null) }
    ...
    LaunchedEffect(targetType, templateId, targetPackageName) {
        ...
        val state = loadEditorState(context, targetType, templateId, targetPackageName)
        apps = installed
        ...
        selected = state.selection ?: installed.mapTo(mutableSetOf()) { it.packageName }
        savedSelection = selected
        listMode = state.listMode
        savedListMode = state.listMode
        loaded = true
    }

    val visibleApps = apps.matching(typeFilter, query)
    val hasChanges = loaded && (selected != savedSelection || listMode != savedListMode)
    ...
    fun save() {
        ...
        when (targetType) {
            TargetType.TEMPLATE -> templateId?.let { id ->
                val templates = PackageVisibilityPrefs.loadTemplates(context).map { template ->
                    if (template.id == id) {
                        template.copy(
                            visiblePackages = selected,
                            listMode = listMode ?: template.listMode
                        )
                    } else {
                        template
                    }
    ...
    fun applyToVisible(shouldSelect: (current: Set<String>, packageName: String) -> Boolean) {
        selected = buildSet {
            addAll(selected)
            visibleApps.forEach { app ->
                if (shouldSelect(selected, app.packageName)) {
                    add(app.packageName)
                } else {
    ...
    Scaffold(
        ...
        topBar = {
            CenterAlignedTopAppBar(
                ...
                actions = {
                    ...
                    SaveConfigIconButton(hasChanges = hasChanges, onClick = { save() })
                },
        ...
        LazyColumn(
            ...
            // Editing a template's list is the moment the whitelist/blacklist
            // question actually comes up, so the switch lives here rather than
            // only behind a menu item on the manager screen.
            listMode?.let { mode ->
                item(key = "list_mode") {
                    ElevatedCard(
                        ...
                        Column(
                            ...
                            Text(
                                text = stringResource(R.string.template_list_mode),
...
/**
 * What the editor starts from: the packages already selected, and — for a
 * template — the list mode it currently uses.
 *
 * A null [selection] means "no restriction recorded", which the caller expands
 * to every installed package rather than to an empty set.
 */
private data class EditorState(
    val selection: Set<String>?,
    val listMode: TemplateListMode?
)
...
private suspend fun loadEditorState(
    ...
    when (targetType) {
        ...
        TargetType.APP -> {
            ...
            EditorState(
                selection = rule?.visiblePackages
